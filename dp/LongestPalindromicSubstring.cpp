...
#include <stdio.h>
#include <string.h>
bool isPalindrome(const char *s,int l,int h)
{
...
int abs(int i,int j) { int k=i-j; if(k<0) return -k; return k;}
int  longestPalSubstr1(const char *s)
{
  int n=strlen(s);
  int max=INT_MIN;
  int mi,mj;
  for(int i=0;i<n;i++){
    for(int j=i+1;j<n;j++){
      if(isPalindrome(s,i,j)){ if(max<abs(i,j)){ max=abs(i,j);mi=i;mj=j;}}
    }
  }
  printSubStr(s,mi,mj);
return abs(mi,mj)+1;
