using System;
using System.Collections.Generic;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Threading.Tasks;
using System.Web.Http;
using MSP2.Models;
using System.Net.Http.Headers;
using MSP2.ParkingMgr;
namespace MSP2.Controllers
{
    public class MSP2Controller : ApiController
    {
        //IList<Employee> employees = new List<Employee>()
        //{
        //        //new Employee()
        //        //{
        //        //    EmployeeId = 5, EmployeeName = "Manish Sharma", Address = "New Delhi", Department = "HR"
        //        //},
        //};
        public string GetAllEmployees()
        {
            //Return list of all employees  
            return "Hello Happy to see me";
        }
        //public Employee GetEmployeeDetails121(string id)
        //{
        //    //Return a single employee detail  
        //    var employee = employees.FirstOrDefault(e => e.EmployeeId == 1);
        //    if (employee == null)
        //    {
        //        throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound));
        //    }
        //    return employee;
        //}
        //public Employee GetEmployeeDetails(int id, int id2)
        //{
        //    //Return a single employee detail  
        //    var employee = employees.FirstOrDefault(e => e.EmployeeId == id);
        //    if (employee == null)
        //    {
        //        throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound));
        //    }
        //    return employee;
        //}

        [HttpPost]
        public async Task<string> HandlePostRequest(ForeignData value)
        {
           
            if (value.operation != null
                && value.operation.CompareTo("login") == 0
                && value.fcm_token != null
                && value.EmpAlias != null
                )
            {
                try
                {
                    ParkingLot pkmgr = ParkingLot.Instance;
                    ParkingLot.fcm_alias[value.EmpAlias] = new Emp(value.EmpAlias, value.fcm_token, 2);// store alias fcm and his preferred building location
                    HttpClient client = new HttpClient();
                    String url = String.Format("https://graph.microsoft.com/v1.0/users?$filter=userPrincipalName eq '{0}@microsoft.com'", value.EmpAlias);
                    //String url = String.Format("http://who/is/{0}", value.EmpAlias);
                    // client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer","..enuxOH_rY-0SJBS7btLVmEGj0jQdi9jQfMgs-K2tITYMYDTLCdixJSV9iIudYfh_CXEOUC97AlchwBoNXexQaQrZTCP3v8mZhJEbP92F6RbazcfAd04Dy9WO8aEVco9ZBpCDYkMxj-Uo1YUDOiRsa3BvhKYWfItSoLBLYKGxkZBaNZzs3Vvk657LlYGDO_ZmoAa_FLM-4m5KA1ch92shHq7pg6lEiuFT2EET7pe81R57Ug0g1vUq2CWFcqQdBwBFHJSo8P9l3eUt-Xh3KnHG74sU2HPJxDXmP83tjikSCybuTXK_a_ll3m7aGyvENoT8RJQGvle6EW-rbCi2LSqVAg");
                    // var responseString = await client.GetStringAsync(url);
                    // parse json and send name and location success
                    return "Login:SUCCESS";
                }
                catch
                {
                    return "Login:FAILED";
                }
            }
            if (value.operation != null
                && value.operation.CompareTo("entry") == 0
                && value.EmpAlias != null
                )
            {
                try
                {
                    ParkingLot pkmgr = ParkingLot.Instance;
                    Vehicle v = new Vehicle("Baleno1", ParkingLot.fcm_alias[value.EmpAlias].buildingLoc);
                    long ret = pkmgr.park(v);
                    ParkingLot.fcm_alias[value.EmpAlias].token = ret;
                    return "Park:SUCCESS";
                }
                catch
                {
                    return "Park:FAILED";
                }
            }
            if (value.operation != null
                 && value.operation.CompareTo("exit") == 0
                    && value.EmpAlias != null
               )
            {
                try
                {
                    ParkingLot pkmgr = ParkingLot.Instance;
                    Vehicle v = pkmgr.unpark(ParkingLot.fcm_alias[value.EmpAlias].token);
                    ParkingLot.fcm_alias[value.EmpAlias].token = 0;
                    return "UnPark:SUCCESS";
                }

                catch
                {
                    return "UnPark:FAILED";
                }
            }
            if (value.operation != null
             && value.operation.CompareTo("reportabuse") == 0
                && value.EmpAlias != null
           )
            {
                // handle code for report abuse
            }

            return value.EmpAlias;
            //write insert logic  


        }

    }
}
